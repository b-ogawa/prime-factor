...
///
/// # Preconditions
/// - `n` は判定対象となる256ビット巨大整数。
///
/// # Postconditions
/// - `n` が素数であれば `true`、合成数であれば `false` を返します。
///   (2^64 までの数に対しては100%正確であり、それ以上でも実用上十分な確率的正確性を持ちます)
...
/// # Preconditions
/// - `p` は奇素数であること。
/// - `n` は法 `p` において平方剰余（`jacobi(n, p) == 1`）であること。
///
/// # Postconditions
/// - 平方根 `r` が存在する場合は `Some(r)` を返し、存在しない場合は `None` を返します。
